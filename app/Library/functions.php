...
function saveNotification( $title, $message, $userId = null, $storeId = null ) {

        $created = Notification::create(['user_id'=> $userId, 'title'=> $title,
            'description'=> $message, 'store_id'=>$storeId]);
        return $created;
...
$user = User::where('id',$user_id)->where('type',2)->first();
    ...
    // store the total rate to client
    $rates =$user->storeRates()->get();
